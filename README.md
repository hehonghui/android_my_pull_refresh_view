...
============================
# 概述
> 这是一个通用的下拉刷新、上拉自动加载的组件，该组件继承自LinearLayout,方向为竖直布局，由三部分组成，分别是Header、ContentView、Foooter,其中的宽高都为match_parent,另外两个都为宽、高分别为match_parent、wrap_content，且Header、Foooter在初始时都会通过设置padding隐藏掉，只有
